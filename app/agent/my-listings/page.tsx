...
const MOCK_LISTINGS: Listing[] = [
  {
    ...
    price: 15000000,
    image:
      "https://images.unsplash.com/photo-1568605114243-e209b1292591?q=80&w=2000&auto=format&fit=crop",
    status: "active",
  ...
  },
  {
    ...
    price: 750000,
    image:
      "https://images.unsplash.com/photo-1580582932707-5205c5f491af?q=80&w=2000&auto=format&fit=crop",
    status: "active",
    createdAt: new Date().toISOString(),
    agentId: "me",
    bedrooms: 4,
...
export default function MyListingsPage() {
        ...
        prev.map((listing) =>
          listing.id === listingId
            ? {
                ...listing,
                isAvailable,
                status: isAvailable ? "active" : ("sold" as const),
              }
            : listing,
        ),
      );
  ...
  const handleReferralToggle = async (
    ...
    setListings((prev) =>
      prev.map((listing) =>
        listing.id === listingId ? { ...listing, referralsOn } : listing,
      ),
    );
  ...
  const handleBulkReferralToggle = () => {
    ...
    toast.success(
      `Toggled referrals for ${selectedListings.size} listing${selectedListings.size > 1 ? "s" : ""}`,
    );
  };
  ...
  const filteredListings = useMemo(() => {
    return listings.filter((listing) => {
      ...
      const matchesReferral =
        ...
        (filters.referralStatus === "on"
          ? listing.referralsOn
          : !listing.referralsOn);
      const matchesUpdate =
        filters.updateStatus === "all" ||
        (filters.updateStatus === "up-to-date"
          ? !listing.isStale
          : listing.isStale);
  ...
  const counts = useMemo(() => {
    ...
    return {
      referralStatus: referralCounts,
      updateStatus: updateCounts,
  ...
  const locations = Array.from(
    new Set(listings.map((l) => l.location.split(",")[0].trim())),
  );
        ...
        <main className="flex-1 h-full overflow-y-auto flex flex-col">
          <div className="px-10 pb-20 pt-2 space-y-6">
              ...
              <div className="flex flex-wrap gap-2 items-center min-h-[32px]">
                {filters.referralStatus !== "all" && (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        handleFilterChange("referralStatus", "all")
                      }
                ...
                {filters.updateStatus !== "all" && (
                  <Badge
                    variant="secondary"
                    className="gap-1 pl-2 pr-1 py-1 bg-orange-100 text-orange-700 hover:bg-orange-200 dark:bg-orange-500/20 dark:text-orange-300"
                  >
                    Status: {filters.updateStatus}
                    <button
                      onClick={() => handleFilterChange("updateStatus", "all")}
                ...
                {filters.availability !== "all" && (
                  <Badge
                    variant="secondary"
                    className="gap-1 pl-2 pr-1 py-1 bg-green-100 text-green-700 hover:bg-green-200 dark:bg-green-500/20 dark:text-green-300"
                  >
                    Availability: {filters.availability}
                    <button
                      onClick={() => handleFilterChange("availability", "all")}
                      className="hover:bg-green-200 dark:hover:bg-green-500/30 rounded-full p-0.5"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </Badge>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              >
                <ToggleRight className="w-5 h-5 text-blue-500" /> Toggle
                Referrals
              </button>
