...
interface DocumentUpload {
    file: File | null;
    preview: string | null;
    status: 'idle' | 'uploading' | 'success' | 'error' | 'pending' | 'approved' | 'rejected';
    error?: string;
...
export default function KYCTab() {
    ...
    useEffect(() => {
        const fetchExistingDocuments = async () => {
            try {
                ...
                const selfieType = types.find(t => t.name.toLowerCase() === "voter's card"); // Temporary workaround
                ...
                if (docs.length > 0) {
                    ...
                    docs.forEach((doc, index) => {
                        const status = doc.status.toLowerCase() as 'pending' | 'approved' | 'rejected';
                        if (index === 0) {
                            setNinDocument({
                                file: null,
                                preview: doc.url,
    ...
    const handleSubmit = async () => {
        ...
        const needsNinUpload = ninDocument.file && (ninDocument.status === 'idle' || ninDocument.status === 'rejected' || ninDocument.status === 'error');
        const needsSelfieUpload = selfieDocument.file && (selfieDocument.status === 'idle' || selfieDocument.status === 'rejected' || selfieDocument.status === 'error');

        // Check if NIN needs upload (not already pending/approved)
        const ninReady = ninDocument.file || ['pending', 'approved'].includes(ninDocument.status);
        const selfieReady = selfieDocument.file || ['pending', 'approved'].includes(selfieDocument.status);
        ...
        try {
            ...
            if (error.message?.includes('already uploaded')) {
                toast.error('You have already submitted these documents. You can only re-upload if they are rejected.');
            } else {
            ...
            if (ninDocument.status === 'uploading') {
                setNinDocument(prev => ({ ...prev, status: 'error', error: error.message }));
            }
            if (selfieDocument.status === 'uploading') {
                setSelfieDocument(prev => ({ ...prev, status: 'error', error: error.message }));
            }
        } finally {
                ...
                >
                    ...
                    {isUploading ? (
                        ...
                        <div className="flex flex-col items-center gap-2 text-yellow-600 dark:text-yellow-400">
                            ...
                            <p className="text-xs">Document submitted, awaiting verification</p>
                        </div>
                    ) : isApproved ? (
    ...
    const ninSubmitted = ['pending', 'approved', 'rejected'].includes(ninDocument.status);
    const selfieSubmitted = ['pending', 'approved', 'rejected'].includes(selfieDocument.status);
    const allSubmitted = ninSubmitted && selfieSubmitted;
    const allApproved = ninDocument.status === 'approved' && selfieDocument.status === 'approved';
    const anyRejected = ninDocument.status === 'rejected' || selfieDocument.status === 'rejected';
    ...
    const hasNewNinFile = ninDocument.file && !['pending', 'approved'].includes(ninDocument.status);
    const hasNewSelfieFile = selfieDocument.file && !['pending', 'approved'].includes(selfieDocument.status);
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-4">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                Upload a clear image or PDF of your National Identification Number (NIN) slip.
                            </p>
                            {renderUploadBox(
                                'NIN Slip',
                        ...
                        <div className="space-y-4">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                Take or upload a clear selfie. Ensure your face is clearly visible and well-lit.
                            </p>
                            {renderUploadBox(
                                'Selfie',
                    ...
                    <div className="pt-8">
                        {allApproved ? (
                            ...
                            <button
                                ...
                                disabled={!canSubmit}
                                className="w-full bg-gradient-to-r from-purple-500 to-blue-500 hover:opacity-90 text-white font-semibold py-3 px-6 rounded-xl flex items-center justify-center gap-2 transition-opacity shadow-lg shadow-purple-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {isSubmitting ? (
                                    <>
                    ...
                    <div className="relative space-y-10">
                        ...
                        <div className="relative">
                            <div className="flex items-start gap-5">
                                <div className={`z-10 w-10 h-10 flex-shrink-0 ${allSubmitted ? 'bg-gradient-to-br from-purple-500 to-blue-500' : 'bg-slate-200 dark:bg-slate-700'} rounded-full flex items-center justify-center shadow-md ${allSubmitted ? 'shadow-purple-500/20' : ''}`}>
                                    <Check className={`w-5 h-5 ${allSubmitted ? 'text-white' : 'text-slate-400'}`} />
                                </div>
                                <div>
                                    <p className="font-semibold text-slate-700 dark:text-slate-200">Documents Submitted</p>
                                    <p className="text-sm text-slate-500 dark:text-slate-400">
                                        {allSubmitted ? 'Your documents have been submitted' : 'Upload and submit your documents'}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="relative">
                            <div className="flex items-start gap-5">
                                <div className={`z-10 w-10 h-10 flex-shrink-0 ${allSubmitted && !allApproved ? 'bg-gradient-to-br from-yellow-500 to-orange-500' : 'bg-slate-200 dark:bg-slate-700'} rounded-full flex items-center justify-center ${allSubmitted && !allApproved ? 'shadow-md shadow-yellow-500/20' : ''}`}>
                                    <Hourglass className={`w-5 h-5 ${allSubmitted && !allApproved ? 'text-white' : 'text-slate-400'}`} />
                                </div>
                                <div>
                                    <p className={`font-semibold ${allSubmitted && !allApproved ? 'text-slate-700 dark:text-slate-200' : 'text-slate-500 dark:text-slate-400'}`}>Under Review</p>
                        ...
                        <div className="relative">
                            <div className="flex items-start gap-5">
                                <div className={`z-10 w-10 h-10 flex-shrink-0 ${allApproved ? 'bg-gradient-to-br from-green-500 to-emerald-500' : 'bg-slate-200 dark:bg-slate-700'} rounded-full flex items-center justify-center ${allApproved ? 'shadow-md shadow-green-500/20' : ''}`}>
                                    <ShieldCheck className={`w-5 h-5 ${allApproved ? 'text-white' : 'text-slate-400'}`} />
                                </div>
                                <div>
                                    <p className={`font-semibold ${allApproved ? 'text-slate-700 dark:text-slate-200' : 'text-slate-500 dark:text-slate-400'}`}>Verification Complete</p>
                                    <p className="text-sm text-slate-400 dark:text-slate-500">
                                        {allApproved ? 'Your account has been verified!' : 'Your account will be updated upon approval.'}
                                    </p>
                                </div>
                            </div>
                        </div>
