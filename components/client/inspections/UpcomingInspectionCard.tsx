...
export default function UpcomingInspectionCard({ date, title, subtitle, agent }: UpcomingInspectionCardProps) {
    return (
        <div className="bg-white/60 backdrop-blur-sm p-6 rounded-2xl shadow-sm border border-white/50 flex flex-col md:flex-row gap-6">
            <div className={`flex flex-col items-center justify-center ${date.bgColor} p-4 rounded-xl w-full md:w-24 flex-shrink-0`}>
                <span className={`text-sm font-semibold ${date.textColor} uppercase`}>{date.month}</span>
